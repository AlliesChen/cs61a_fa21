...
def product(n: int, term: Callable[[int], int]):
...
def accumulate(merger: Callable[[int, int], int], base: int, n: int, term: Callable[[int], int]):
    ...
    accum = base
    for i in range(1, n + 1):
        accum = merger(accum, term(i))

    return accum


def summation_using_accumulate(n: int, term: Callable[[int], int]):
...
def product_using_accumulate(n: int, term: Callable[[int], int]):
