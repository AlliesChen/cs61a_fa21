class VendingMachine:
    ...
    def vend(self):
        if (self.stock == 0):
            ...
            self.stock -= 1
            return f'Here is your {self.prod}.'
        else:
            self.stock -= 1
            change = self.funds - self.price
            self.funds = 0
            return f'Here is your {self.prod} and ${change} change.'
    ...
    def restock(self, amt):
        self.stock += amt
        return f'Current {self.prod} stock: {self.stock}'
...
def store_digits(n):
    ...
    """
    # refer to https://github.com/y1cunhui/cs61A-2021Fall/blob/main/hw06/hw06.py
    tail = Link.empty
    while n != 0:
        tail = Link(n % 10, tail)
        n //= 10
...
def two_list(vals, amounts):
    ...
    for i in range(len(vals)):
        for _ in range(amounts[i]):
            if i == 0 and head is Link.empty:
                head = Link(vals[i], tail)
                tail = head
            else:
                tail.rest = Link(vals[i], Link.empty)
                tail = tail.rest
    return head
...
class VirFib():
    ...
    def next(self):
        if self.value == 0:
            ...
            fib = VirFib(self.value + self.previous)
...
def bst_max(t):
    ...
    if t.is_leaf():
        return t.label
    return max([bst_max(x) for x in t.branches])
...
def bst_min(t):
    ...
    if t.is_leaf():
        return t.label
    return min([bst_min(x) for x in t.branches])
...
def is_bst(t):
    ...
    False
    """
    # reference: https://github.com/y1cunhui/cs61A-2021Fall/blob/main/hw06/hw06.py
    if t.is_leaf():
        return True
