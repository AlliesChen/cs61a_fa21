...
def roll_dice(num_rolls: int, dice=six_sided):
    ...
    lock = False
    accum = 0
    for iterator in range(num_rolls):
        result = dice()
        if lock == False and result != 1:
            accum += result
...
def picky_piggy(score: int):
    ...
    match nth:
        ...
        case 0:
            return 7
        case _:
            raise Exception(f'{nth} does not match any case!')
    # END PROBLEM 2


def take_turn(num_rolls: int, opponent_score: int, dice=six_sided, goal=GOAL_SCORE):
...
def hog_pile(player_score: int, opponent_score: int):
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        running_score = 0
        if who == 0:
            running_score += score0
        if who == 1:
            running_score += score1
...
def make_averaged(original_function: Callable[[tuple], int], trials_count=1000):
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num_rolls in range(1, 11):
        ...
        if score > highest_score:
            highest_score, dice_roll = score, num_rolls
    return dice_roll
    # END PROBLEM 9
